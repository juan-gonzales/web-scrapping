...
async function loginWithConfig(options: InternalOptions): Promise<LoginResult> {
  const { browser, config, credentials, evidenceDir, genericCaptchaSelectors, onLog, requestCaptcha, appKey } = options;
  ...
  try {
    ...
    await page.goto(config.loginUrl, { timeout: config.timeoutsMs.goto, waitUntil: 'domcontentloaded' });
    ...
    const captchaHandle = await detectCaptcha(page, selectors.captchaImage, genericCaptchaSelectors);
    ...
    if (captchaHandle) {
      if (!selectors.captchaInput) {
        return {
          ...
          message: 'Se detectó CAPTCHA pero no hay selector configurado para el input.',
        };
      }

      log('CAPTCHA detectado, esperando texto ingresado por el usuario.');
      const captchaBuffer = await captchaHandle.screenshot();
      const captchaText = requestCaptcha ? await requestCaptcha(captchaBuffer) : undefined;
      ...
      await fillInput(page, selectors.captchaInput, captchaText, 'captcha', log, true);
    }
    ...
    if (anchorsFound.length === 0) {
      return {
        ...
        message: 'No se encontraron los selectores ancla configurados después del login.',
      };
    }
    ...
    const screenshotPath = path.join(evidenceDir, `${appKey}-screenshot-${Date.now()}.png`);
    ...
    return {
      ...
      screenshotPath,
      screenshotThumb: `data:image/png;base64,${screenshotBuffer.toString('base64')}`,
    };
  } catch (error) {
...
async function fillInput(page: Page, selector: string, value: string, label: string, log: (msg: string) => void, clear = false) {
  if (!selector) {
...
async function detectCaptcha(page: Page, captchaSelector?: string, genericSelectors?: string[]) {
  if (captchaSelector) {
...
async function waitForPostLogin(page: Page, config: MelConfig, log: (msg: string) => void) {
  log('Esperando navegación post login.');
  try {
    await page.waitForNavigation({ timeout: config.timeoutsMs.goto, waitUntil: 'domcontentloaded' });
  } catch (error) {
...
async function collectAnchors(
  ...
  const timeoutPerAnchor = anchors.length > 0 ? Math.max(2000, Math.floor(totalTimeout / anchors.length)) : totalTimeout;
  await page.waitForTimeout(1000);
  for (const anchor of anchors) {
    try {
      const element = await page.waitForSelector(anchor, { timeout: timeoutPerAnchor, state: 'attached' });
      if (element) {
